...
#include <ctime>
#include <algorithm>
#include "../../../modules/task_3/zhbanova_n_matrix_multiplication_tbb/matrix_multiplication.h"
...
void getParallelTBBMatrixMultiplication(const SparseMatrix<CCS>& A, const SparseMatrix<CCS>& B, SparseMatrix<CCS>* C,
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, A.getMatrixSize(), 1), [&](const tbb::blocked_range<size_t>& k) {
        for (size_t n = k.begin(); n != k.end(); ++n) {
            size_t lj = n;
            // printf("n - %ld\n", n);
            size_t iterator = 0;
            size_t li = 0;
